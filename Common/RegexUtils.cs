...
namespace Common
{
    public class RegexUtil
    {
        ...
        public static bool IsEmail(string input)
        {
            ...
            string pattern = @"^[a-zA-Z\d]+([-_.][a-zA-Z\d]+)*@(?:[-A-Za-z0-9]+\.)+[A-Za-z]{2,6}$";
            return IsMatch(pattern, input);
        }
        ...
        {
            //string pattern = @"^([0-9A-F]{2}[:-]){5}[0-9A-F]{2}|([0-9A-F]{2}){6}$";
            string pattern = @"^([0-9A-F]{2}[-]){5}[0-9A-F]{2}|([0-9A-F]{2}[:]){5}[0-9A-F]{2}|([0-9A-F]{2}){6}$";
            return IsMatch(pattern, input);
        }
        ...
        public static decimal AddZoro(decimal v, int dotLength)
        {
            string t = v.ToString();
            string[] tt = t.Split('.');
            if (tt.Length == 1)
                return Convert.ToDecimal(t + ".00");

            if (tt[1].Length < dotLength)
            {
                for (int i = 0; i < (dotLength - (dotLength - tt[1].Length)); i++)
                    t += "0";
